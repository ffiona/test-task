...
modulePath = `${dirPath}/${args.name}/${args.name}.twig`;
...
if (fs.existsSync(modulePath)) {
    ...
    const config = {
        fileSave  : true,
        filePath  : path.resolve(__dirname, `../src/components/${args.name}/${args.name}.scss`),
        overwrite : true,
        eol       : '\n',
        nested    : false,
        curlbraces: true,
    ...
    console.error('\x1b[31m%s\x1b[0m', `${args.name} - такого компонента не существует`);
}
